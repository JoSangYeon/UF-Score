...
class VisionLlama7BCaptionModel(BaseCaptioning):
    def __init__(self, config):
        super().__init__(config)
        
        self.prompt = [
            "Describe the detail of this image:<AcousticTokens>\n---\nDetailed: "
        ]

        # encoder
        # self.encoder = Blip2VisionModel.from_pretrained("Salesforce/blip2-opt-2.7b",
        #                                                 torch_dtype=torch.bfloat16,)
    ...
    def forward_encoder(self, audios):
        audio_embeds = self.encoder(audios)[0]
    ...
    def forward(self, samples):
        ...
        prompt = [random.choice(self.prompt)] * len(text)
        # encoder
        encoder_hidden_states, encoder_atts = self.forward_encoder(images)
    ...
    def generate(
        ...
        with autocast():
            images = samples["images"].to(self.device)

            prompt = [random.choice(self.prompt)] * images.shape[0]
            encoder_hidden_states, encoder_atts = self.forward_encoder(images)
            input_embeds, input_mask, decoder_targets = (
                self.prepare_inputs_labels_for_multimodal(
                    encoder_hidden_states, encoder_atts, prompt
